...
def readfile(path, type='xls'):
    if type == 'xls':
        ...
        try:
            filename = re.match(r'^([0-9a-zA-Z/:_.\u4e00-\u9fa5]+)/'  # 父文件夹
                                    r'([0-9a-zA-Z]+)_([0-9a-zA-Z]+)_([0-9a-zA-Z_]+).(csv)$', path)  # 文件名
            CarName = filename.group(2)
            TestName = filename.group(3)
            DriverName = filename.group(4)
            Testid_com = 1
            ...
            DriverName = 'Unknown'
            Testid_com = 1
        csvdata = pd.read_csv(path,encoding='gbk')
...
def read_file(file_path, DBC_index, Car_index, Driver_index):
    ...
    # path_list = glob.glob(file_path)  # 获取文件目录下所有的文件
    Travel_ID = 0

    for i in file_path:
        type = re.match(r'^([0-9a-zA-Z/:_\u4e00-\u9fa5]+).([a-zA-Z]+)$', i).group(2)  # 后缀
        ...
        yield message, file_path.__len__(), Combine_file_name, CarName


def data_process(file_path, save_name, pt_type='traditional'):
    # --------------------------- Main Scripts  -------------------------------
    ...
    for i in range(Target_Driver_Date.shape[0]):  # 样本数据个数
        ...
        if Target_Driver.shape[0] > 1000:  # 太短的样本选择丢弃
            ...
            vspd = Target_Driver['Veh_Spd_NonDrvn']
            ...
            En_spd = Target_Driver['En_Spd']
            time = range(len(Fuel_Csump))
            X_acc = five_points_avg_acc(vspd.tolist())
            ...
            # ------------------- Style features ---------------------
            odemeter = odemeter_cal(vspd.tolist())
            ...
            overtake_times = 0
            brake_skill_score = brake_skill(VSE_Longt_Acc, vspd.tolist())
            sudden_acc_times = sudden_acc(X_acc, vspd.tolist(), CarName)
            ...
            over_speed_propotion = over_speed_cal(Target_Driver, vspd.tolist())
            ...
            idle_time = Target_Driver[(vspd == 0) & (En_spd > 0)].shape[0] / 10
            run_time = Target_Driver[vspd > 0].shape[0] / 10
            total_time = Target_Driver.shape[0] / 10
            Fuel_Csump_idle = Target_Driver[(vspd == 0) & (En_spd > 0)]['Fuel_Csump']
            try:
            ...
            start_times = start_times_cal(vspd)
            ...
            resultarray.append([int(DriverID), int(Date), int(Time_index_start), brake_skill_score, sudden_acc_score,
    ...
    field_name = np.array(['DriveID', 'Date', 'Time_start', 'Name',
                           ...
                           'FuelCons', 'Soc_balance_ratio', 'Soc_start', 'Soc_end', 'Run_time', 'Total_time',
                           'FC_idle[l/h]', 'Start_times'])
